...
void parseCpuUsagesFileAndAssignUsages(std::vector<CpuUsage> *cpu_usages) {
        ...
        if (line.find("cpu") == 0 && isdigit(line[3])) {
            ...
            if (cpu_num < kMaxCpus) {
                ...
                std::string cpu_online_path =
                        ::android::base::StringPrintf("%s/%s/%s", kCpuOnlineRoot.data(), cpu_name.c_str(),
...
bool ThermalHelper::fillCpuUsages(std::vector<CpuUsage> *cpu_usages) const {
