...
bool RenderTopView::activate() {
    ...
    // Load the car image
    mTexAssets.carTopView.reset(createTextureFromPng("/system/etc/automotive/evs/ImxCarFromTop.png"));
    ...
    for (auto&& cam : mActiveCameras) {
        cam.tex.reset(
                createVideoTexture(mEnumerator, cam.info.cameraId.c_str(), mTargetCfg, sDisplay));
        if (!cam.tex) {
